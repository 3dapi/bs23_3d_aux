...
CD3DApplication::CD3DApplication()
{
	...
	strcpy(m_strClassName, "D3D9 Application");
...
BOOL CD3DApplication::ConfirmDeviceHelper( D3DCAPS9* pCaps, INT vertexProcessingType, void* pDeviceCombo)
{
	DWORD dwBehavior;

	if (vertexProcessingType == CD3DEnumeration::SOFTWARE_VP)
		dwBehavior = D3DCREATE_SOFTWARE_VERTEXPROCESSING;
	else if (vertexProcessingType == CD3DEnumeration::MIXED_VP)
		dwBehavior = D3DCREATE_MIXED_VERTEXPROCESSING;
	else if (vertexProcessingType == CD3DEnumeration::HARDWARE_VP)
		dwBehavior = D3DCREATE_HARDWARE_VERTEXPROCESSING;
	else if (vertexProcessingType == CD3DEnumeration::PURE_HARDWARE_VP)
		dwBehavior = D3DCREATE_HARDWARE_VERTEXPROCESSING | D3DCREATE_PUREDEVICE;
	else
		dwBehavior = 0; // TODO: throw exception

	D3DFORMAT AdapterFormat = ((CD3DEnumeration::D3DDeviceCombo*)pDeviceCombo)->AdapterFormat;
	D3DFORMAT BackBufferFormat = ((CD3DEnumeration::D3DDeviceCombo*)pDeviceCombo)->BackBufferFormat;
...
HRESULT CD3DApplication::ConfirmDevice( D3DCAPS9* pCaps, DWORD dwBehavior
									   , D3DFORMAT adapterFormat, D3DFORMAT backBufferFormat)
{
	...
	//
	//		// 픽셀 쉬이더 버전은 1.1
	//		if( pCaps->PixelShaderVersion < D3DVS_VERSION(1,1) )
	//			return E_FAIL;
	//	}
...
HRESULT CD3DApplication::Create( HINSTANCE hInstance )
{
	...
	if( m_hWnd == NULL)
	{
		...
		WNDCLASS wc =
		{
			...
			, 0
			, m_hInst
			, LoadIcon( m_hInst, MAKEINTRESOURCE(IDI_MAIN_ICON) )
		...
		AdjustWindowRect( &rc, m_dwWindowStyle, hMenu? TRUE: FALSE);
...
BOOL CD3DApplication::FindBestWindowedMode( BOOL bRequireHAL, BOOL bRequireREF )
{
	...
	CD3DEnumeration::D3DAdapterInfo* pBestAdapterInfo = NULL;
	CD3DEnumeration::D3DDeviceInfo* pBestDeviceInfo = NULL;
	CD3DEnumeration::D3DDeviceCombo* pBestDeviceCombo = NULL;

	for( UINT iai = 0; iai < m_d3dEnumeration.m_pAdapterInfoList->Count(); iai++ )
	{
		CD3DEnumeration::D3DAdapterInfo* pAdapterInfo = (CD3DEnumeration::D3DAdapterInfo*)m_d3dEnumeration.m_pAdapterInfoList->GetPtr(iai);
		for( UINT idi = 0; idi < pAdapterInfo->pDeviceInfoList->Count(); idi++ )
		{
			CD3DEnumeration::D3DDeviceInfo* pDeviceInfo = (CD3DEnumeration::D3DDeviceInfo*)pAdapterInfo->pDeviceInfoList->GetPtr(idi);
			...
			{
				CD3DEnumeration::D3DDeviceCombo* pDeviceCombo = (CD3DEnumeration::D3DDeviceCombo*)pDeviceInfo->pDeviceComboList->GetPtr(idc);
...
EndWindowedDeviceComboSearch:
	...
	m_d3dSettings.Windowed_VertexProcessingType = *(CD3DEnumeration::VertexProcessingType*)pBestDeviceCombo->pVertexProcessingTypeList->GetPtr(0);
...
EndFullscreenDeviceComboSearch:
	...
	INT					idm = 0;
	INT					nDisplayModeList = pBestAdapterInfo->pDisplayModeList->Count();
	D3DDISPLAYMODE*		pdm = NULL;

	for(idm=0; idm<nDisplayModeList; ++idm)
	{
		pdm = (D3DDISPLAYMODE*)pBestAdapterInfo->pDisplayModeList->GetPtr(idm);

		if( pdm->Format != pBestDeviceCombo->AdapterFormat )
			continue;

		if( pdm->Width == this->m_dwCreationWidth &&
			pdm->Height == this->m_dwCreationHeight &&
		...
		else if( pdm->Width == this->m_dwCreationWidth &&
			pdm->Height == this->m_dwCreationHeight &&
	...
	m_d3dSettings.Fullscreen_VertexProcessingType = *(CD3DEnumeration::VertexProcessingType*)pBestDeviceCombo->pVertexProcessingTypeList->GetPtr(0);
...
LRESULT WINAPI CD3DApplication::WndProc(HWND hWnd,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
...
HRESULT CD3DApplication::Initialize3DEnvironment()
{
	...
	CD3DEnumeration::D3DAdapterInfo* pAdapterInfo = m_d3dSettings.PAdapterInfo();
	CD3DEnumeration::D3DDeviceInfo* pDeviceInfo = m_d3dSettings.PDeviceInfo();
	...
	DWORD behaviorFlags;
	if (m_d3dSettings.GetVertexProcessingType() == CD3DEnumeration::SOFTWARE_VP)
		behaviorFlags = D3DCREATE_SOFTWARE_VERTEXPROCESSING;
	else if (m_d3dSettings.GetVertexProcessingType() == CD3DEnumeration::MIXED_VP)
		behaviorFlags = D3DCREATE_MIXED_VERTEXPROCESSING;
	else if (m_d3dSettings.GetVertexProcessingType() == CD3DEnumeration::HARDWARE_VP)
		behaviorFlags = D3DCREATE_HARDWARE_VERTEXPROCESSING;
	else if (m_d3dSettings.GetVertexProcessingType() == CD3DEnumeration::PURE_HARDWARE_VP)
	...
	if( SUCCEEDED(hr) )
	{
		...
		m_pd3dBackBuffer->GetDesc( &m_d3dsdBackBuffer );
		...
		D3DXCreateSprite(m_pd3dDevice, &m_pd3dSprite); 	// Sprite create
...
HRESULT CD3DApplication::DisplayErrorMsg( HRESULT hr, DWORD dwType )
{
	...
	if( MSGERR_APPMUSTEXIT == dwType )
	{
		...
		MessageBox( NULL, strMsg, m_strClassName, MB_ICONERROR|MB_OK );
	...
	else
	{
		...
		MessageBox( NULL, strMsg, m_strClassName, MB_ICONWARNING|MB_OK );
	}

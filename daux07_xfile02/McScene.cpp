...
{
	SAFE_DELETE(	m_pXClone1		);
	SAFE_DELETE(	m_pXClone2		);
	SAFE_DELETE(	m_pXClone3		);
	SAFE_DELETE(	m_pXOrg			);
}
...
INT	CMcScene::FrameMove()
{
	...
	D3DXMatrixScaling(&mtWld1, 17, 17, 17);
	D3DXMatrixScaling(&mtWld2, 11, 11, 11);
	D3DXMatrixScaling(&mtWld3, 14, 14, 14);

	mtWld1._41 = -15;
	mtWld2._41 =   0;
	mtWld3._41 =  15;

	m_pXClone1->SetWorldMatrix(&mtWld1);
	m_pXClone2->SetWorldMatrix(&mtWld2);
	m_pXClone3->SetWorldMatrix(&mtWld3);


	SAFE_FRAMEMOVE(	m_pXClone1		);
	SAFE_FRAMEMOVE(	m_pXClone2		);
	SAFE_FRAMEMOVE(	m_pXClone3		);
...
void CMcScene::Render()
{
	...
	SAFE_RENDER(	m_pXClone1		);
	SAFE_RENDER(	m_pXClone2		);
	SAFE_RENDER(	m_pXClone3		);

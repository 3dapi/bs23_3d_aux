...
HRESULT CMain::Init()
{
	...
	hr = LcxCreateTextureFromFile(&m_pTex0, m_pd3dDevice, "texture/animate.bmp", D3DX_FILTER_NONE, D3DXCOLOR(0,0,0,1));
	if(FAILED(hr))
		return -1;


	hr = LcxCreateTextureFromFile(&m_pTex1, m_pd3dDevice, "texture/Sprites.dds", D3DX_FILTER_NONE);
	if(FAILED(hr))
		return -1;
...
//	RenderXYZ();
	...
	{
		RECT rcDrw={256,  0, 384, 128};
		D3DXVECTOR3 vcPos(400, 150, 0);

		DWORD color = D3DXCOLOR(1, 1, 1, 1.F);
		m_pLcSprite->Draw(m_pTex1->GetTexture(), &rcDrw, (FLOAT*)&vcPos, color, (FLOAT*)&vcPos, 0);
		m_pLcSprite->Draw(m_pTex1->GetTexture(), &rcDrw, (FLOAT*)&vcPos, color, (FLOAT*)&vcPos, 90);
		m_pLcSprite->Draw(m_pTex1->GetTexture(), &rcDrw, (FLOAT*)&vcPos, color, (FLOAT*)&vcPos, 180);
		m_pLcSprite->Draw(m_pTex1->GetTexture(), &rcDrw, (FLOAT*)&vcPos, color, (FLOAT*)&vcPos, 270);
	}

	{
		RECT rcDrw={128,  0, 256, 128};
		D3DXVECTOR3 vcPos(150, 200, 0);


		m_pLcSprite->Draw(m_pTex1->GetTexture(), &rcDrw, (FLOAT*)&vcPos, 0xFFFFFFFF, (FLOAT*)&vcPos);


		float fT = GetTickCount() / 60.f;

		m_pLcSprite->Draw(m_pTex1->GetTexture(), &rcDrw, (FLOAT*)&vcPos, 0xFFFFFFFF, (FLOAT*)&D3DXVECTOR2(vcPos.x + 0, vcPos.y + 0), fT);

...
HRESULT CMain::Init()
{
	...
	hr = LcxCreateTextureFromFile(&m_pTex0, m_pd3dDevice, "texture/dx5_logo.bmp", D3DX_FILTER_NONE, 0);
	if(FAILED(hr))
		return -1;


	hr = LcxCreateTextureFromFile(&m_pTex1, m_pd3dDevice, "texture/tile.png", D3DX_FILTER_NONE, 0);
	if(FAILED(hr))
		return -1;
...
//	RenderXYZ();
	...
	{
		RECT rcDrw={0, 0, m_pTex0->GetImgW(), m_pTex0->GetImgH()};
		D3DXVECTOR3 vcPos(140, 140, 0);

		DWORD color = D3DXCOLOR(1, 1, 1, 1.F);
		m_pLcSprite->Draw(m_pTex0->GetTexture(), &rcDrw, (FLOAT*)&vcPos, 0xFFFFFFFF, (FLOAT*)&vcPos, 0  );
		m_pLcSprite->Draw(m_pTex0->GetTexture(), &rcDrw, (FLOAT*)&vcPos, 0xFFFFFFFF, (FLOAT*)&vcPos, 90 );
		m_pLcSprite->Draw(m_pTex0->GetTexture(), &rcDrw, (FLOAT*)&vcPos, 0xFFFFFFFF, (FLOAT*)&vcPos, 180);
		m_pLcSprite->Draw(m_pTex0->GetTexture(), &rcDrw, (FLOAT*)&vcPos, 0xFFFFFFFF, (FLOAT*)&vcPos, 270);

		vcPos =D3DXVECTOR3(500, 350, 0);
		m_pLcSprite->DrawEx(m_pTex0->GetTexture(), m_pTex1->GetTexture(), &rcDrw, (FLOAT*)&vcPos, 0xFFFFFFFF, (FLOAT*)&vcPos, 0 , (FLOAT*)&D3DXVECTOR2( 2.f, 2.f));

		vcPos =D3DXVECTOR3(250, 350, 0);
		m_pLcSprite->DrawEx(m_pTex0->GetTexture(), m_pTex1->GetTexture(), &rcDrw, (FLOAT*)&vcPos, 0xFFFF0000, (FLOAT*)&vcPos, 0 , (FLOAT*)&D3DXVECTOR2(-2.f, 2.f));

		vcPos =D3DXVECTOR3(500, 150, 0);
		m_pLcSprite->DrawEx(m_pTex0->GetTexture(), m_pTex1->GetTexture(), &rcDrw, (FLOAT*)&vcPos, 0xFF00FF00, (FLOAT*)&vcPos, 0 , (FLOAT*)&D3DXVECTOR2( 2.f,-2.f));

		vcPos =D3DXVECTOR3(250, 150, 0);
		m_pLcSprite->DrawEx(m_pTex0->GetTexture(), m_pTex1->GetTexture(), &rcDrw, (FLOAT*)&vcPos, 0xFF0000FF, (FLOAT*)&vcPos, 0 , (FLOAT*)&D3DXVECTOR2(-2.f,-2.f));
	}

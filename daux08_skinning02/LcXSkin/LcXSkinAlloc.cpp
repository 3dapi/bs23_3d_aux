	...
	if (NumMaterials > 0)            
	{
		...
		for (iMaterial = 0; iMaterial < NumMaterials; iMaterial++)
		{
			if (pMeshContainer->pMaterials[iMaterial].pTextureFilename != NULL)
			{
				...
				if( strlen(tszFilename)>2)
				{
					sprintf(sFileFullPath, "%s%s%s", drive, sPath, tszFilename);
				}
				

				if( FAILED( D3DXCreateTextureFromFile( pd3dDevice, sFileFullPath, &pMeshContainer->ppTextures[iMaterial] ) ) )
